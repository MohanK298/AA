...
{
    /// <summary>
    /// Description of delete_payment_request.
    /// </summary>
    [TestModule("F2B6B4D7-C5AE-4A53-BD8C-61D0992D6053", ModuleType.UserCode, 1)]
    public class delete_payment_request : ITestModule
    {
        /// <summary>
        /// Constructs a new instance.
        /// </summary>
        public delete_payment_request()
        {
    	...
    	private void CreateBillforTE()
    	{
        	...
        	bill.BillingDetailForm.btnSendtoFinal.Click();
     		...
     		if(bill.OutputPromptForm.SelfInfo.Exists(15000))
     		{
    	...
    	private void validateDraftMailWithAPXwithInvalidlink()
    	{
    		txt=outlook.Outlook.DraftsFirstMail.Element.GetAttributeValueText("Name");
			indx1=txt.IndexOf("Subject ")+8;
			indx2=txt.IndexOf(", Sent");
        	...
        	if(outlook.DetailedView.SelfInfo.Exists(5000))
        	{
        		...
        		Delay.Seconds(5);
        		Validate.Exists(bill.APXPayment.lblInvalidLinkInfo,"Invalid Link Exists as expected");
        		Validate.Exists(bill.APXPayment.lblInvalidLnkMsgInfo,String.Format("{0} is seen as expected",bill.APXPayment.lblInvalidLnkMsg.GetAttributeValue<String>("InnerText")));
    	...
    	private void DeletePaymentRequest()
    	{
    		...
    		{
    			Report.Success("Remove/Delete Payment Request Button is seen as expected for Bill without Payment Request Set");
    			bill.MainForm.Toolbar.btnRemovePaymentRequest.Click();
    		}
    		else
    		{
    			Report.Failure("Remove/Delete Payment Request Button is not seen as expected for Bill without Payment Request Set");
    		}
    		...
    		//cmn.VerifyDataNotExistsInTable(bill.MainForm.tblBilling,clientName,"Billing Table");
        ...
        void ITestModule.Run()
        {
            ...
            validateDraftMailWithAPXwithInvalidlink();
            cmn.ClosePrompt();
        }
    }
}
